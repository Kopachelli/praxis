...
`GET /`, `GET /healthz`, and signed `POST /webhook` are public at the application boundary. The read surfaces `GET /incidents`, `GET /incidents/{id}`, `GET /incidents/{id}/memory-match`, and `GET /session` accept exactly one `Authorization: Bearer <token>` header that matches **either** the operator token **or** the separate read-only viewer token (ADR-029). The mutation surface `POST /incidents/{id}/approve` requires the operator token only; a viewer token is rejected there with the same fixed challenge, so read access never confers execution authority. When the deployment sets `PRAXIS_PUBLIC_DEMO_READS` (ADR-031), those same four read surfaces additionally admit an anonymous caller as a read-only `viewer` — no token required — for a public demo; `POST /incidents/{id}/approve` and signed `POST /webhook` are unaffected, so every mutation still requires the operator token or a valid signature. The flag defaults off, preserving the token-protected reads above.
