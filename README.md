...
- **Open the dashboard — no login needed (ADR-031):** the deployed demo serves the incident dashboard **read-only to anyone**. Just open the live app and it auto-loads a read-only view of the incidents, plans, and full decision trails — no token, no signup. You cannot approve, reject, or edit anything: every state-changing action stays behind the operator token, which only the maintainer holds, and the server rejects any mutation without it. (This public-read mode is an opt-in flag for the demo; other deployments keep reads token-protected by default under ADR-025/029.)
- **Verify the deployment yourself:** `GET /healthz` returns the deployed model, version, lifecycle bounds, and readiness flags (public, no token); `POST /incidents/{id}/approve` requires the operator token, and unsigned `POST /webhook` is rejected — so you can confirm the mutation boundary holds even though reads are open.
